...
const swamp_value* swamp_allocator_alloc_struct_create(swamp_allocator* self, const swamp_value** const items, size_t field_count)
{
											   ...
											   const swamp_value** constant_parameters, size_t constant_parameter_count)
{
	...
	const swamp_value* curry_func_value =
		swamp_allocator_alloc_function(self, func->opcodes, func->opcode_count, constant_parameter_count, new_parameter_count,
...
const swamp_value* swamp_allocator_alloc_blob(swamp_allocator* self, const uint8_t* str, size_t octet_count, uint32_t hash)
{
...
const swamp_blob* swamp_allocator_alloc_blob_ex(swamp_allocator* self, const uint8_t* str, size_t octet_count, uint32_t hash)
{
...
const swamp_value* swamp_allocator_alloc_enum_single_value(swamp_allocator* self, uint8_t enum_type, const swamp_value* value)
{
...
const swamp_list* swamp_allocator_alloc_list_conj(swamp_allocator* self, const swamp_list* next_list, const swamp_value* item)
{
	swamp_list* t = calloc(1, sizeof(swamp_list));
	t->value = item;
	INC_REF(item);
...
void swamp_allocator_set_function(swamp_func* t, const uint8_t* opcodes, size_t opcode_count, size_t constant_parameters,
								  size_t parameter_count, size_t register_count, const swamp_value** constants,
												  ...
												  const swamp_value** constants, size_t constant_count, const char* debug_name)
{
	...
	swamp_allocator_set_function(t, opcodes, opcode_count, constant_parameters, parameter_count, register_count, constants,
								 constant_count, debug_name);
	return (const swamp_value*) t;
}

const swamp_value* swamp_allocator_alloc_external_function(swamp_allocator* self, swamp_external_fn fn, size_t parameter_count,
