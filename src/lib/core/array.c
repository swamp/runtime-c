...
SWAMP_FUNCTION_EXPOSE(swamp_core_array_set)
{
	...
	swamp_struct* duplicate = (swamp_struct*) swamp_allocator_alloc_struct_create(allocator, (const swamp_value**) array->fields, array->info.field_count);
...
SWAMP_FUNCTION_EXPOSE(swamp_core_array_push)
{
	...
	swamp_struct* duplicate = (swamp_struct*) swamp_allocator_alloc_struct_create(allocator, (const swamp_value**)array->fields, array->info.field_count + 1);
...
{
	const swamp_struct* array = swamp_value_struct(arguments[0]);
	const swamp_value* list = (const swamp_value*) swamp_allocator_alloc_list_create(allocator, (const swamp_value**) array->fields, array->info.field_count);
...
SWAMP_FUNCTION_EXPOSE(swamp_core_array_slice)
{
	...
	const swamp_value* resultArray = (const swamp_value*) swamp_allocator_alloc_struct_create(allocator, (const swamp_value**) &array->fields[startIndex], count);
