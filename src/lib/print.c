...
static void swamp_value_print_ex(FILE* output, const swamp_value* v, size_t tab_count)
{
	...
	switch (v->internal.type) {
		...
		case swamp_type_string: {
			SWAMP_LOG_FILE_NL(output, "string: '%s' refcount:%zu", ((const swamp_string*) v)->characters, v->internal.ref_count);
		} break;
		case swamp_type_struct: {
			const swamp_struct* s = (const swamp_struct*) v;
			SWAMP_LOG_FILE_NL(output, "struct: field_count: %zu refcount:%zu", s->info.field_count, s->internal.ref_count);
		...
		case swamp_type_function: {
			...
			SWAMP_LOG_FILE_NL(output, "function '%s' parameter_count:%zu constant:%zu (totalreg:%zu, everything:%zu)",
							  func->debug_name, func->parameter_count, func->constant_count, func->total_register_count_used,
							  func->total_register_and_constant_count_used);
		...
		case swamp_type_external_function: {
			...
			SWAMP_LOG_FILE_NL(output, "external function '%s' parameter_count:%zu", func->debug_name, func->parameter_count);
		} break;
