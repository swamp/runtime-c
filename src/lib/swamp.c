...
static const char* g_swamp_opcode_names[] = {
	"nop",	"crs", "upd", "get", "lr",	"conj", "case",	 "brfa",	  "jump",	 "call", "ret",			 "calle", "tail",
	"add",	"sub", "mul", "div", "eql", "ne",	"less",	 "lessequal", "greater", "gte",	 "band",		 "bor",	  "bxor",
	"bnot", "not", "n/a", "n/a", "n/a", "n/a",	"curry", "crl",		  "lap",	 "cre",	 "stringappend", "fxmul", "fxdiv"};
...
#define SET_ENUM_PARAMETER_FORCE_MUTABLE(instance, field_index, value) ((swamp_enum*) instance)->fields[field_index] = value;
							 ...
							 size_t run_parameter_count, swamp_bool verbose_flag)
{
	...
	if (run_parameter_count != f->parameter_count) {
		...
		SWAMP_LOG_INFO("mismatch! param count %zu vs function says %zu (curry %zu) constant count:%zu (%p, %s)",
					   run_parameter_count, f->parameter_count, f->constant_parameter_count, f->constant_count, (const void*) f,
					   f->debug_name);
	...
	if (verbose_flag) {
		SWAMP_LOG_INFO("RUN: total_register_count:%zu\n with constants:%zu opcodecount:%zu", f->total_register_count_used,
					   f->total_register_and_constant_count_used, f->opcode_count);
	}
		...
		switch (*pc++) {
			...
			case swamp_opcode_enum_case: {
				...
				for (uint8_t case_index = 0; case_index < case_count; ++case_index) {
					...
					// SWAMP_LOG_INFO("...case %02X count:%d rel:%02X jump_addr:%04X", enum_type, arg_count, rel_jump_case,
					// absolute_jump_addr);
					if (!found) {
			...
			case swamp_opcode_return: {
				...
				size_t total = call_stack_entry->func->total_register_and_constant_count_used + reserved_return_register_count;
				if (verbose_flag) {
			...
			case swamp_opcode_create_list: {
				...
				const swamp_value* instance =
					(const swamp_value*) swamp_allocator_alloc_list_create(allocator, temp_values, field_count);
				SET_REGISTER(context, target_register, instance);
			} break;
			case swamp_opcode_create_enum: {
				...
				uint8_t field_count = *pc++;
				const swamp_value* instance = (const swamp_value*) swamp_allocator_alloc_enum(allocator, enum_value, field_count);
				for (uint8_t i = 0; i < field_count; ++i) {
					uint8_t copy_from_reg = *pc++;
			...
			case swamp_opcode_list_conj: {
				...
				const swamp_list* swamp_new_list_value =
					swamp_allocator_alloc_list_conj(allocator, (const swamp_list*) swamp_list_value, item);
				SET_REGISTER(context, target_register, (const swamp_value*) swamp_new_list_value);
			} break;
